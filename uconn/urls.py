...
from schedr.uconn.models import Term, Major, Course, Section, Location, UserData, Final
from schedr.base.models import School
from schedr.uconn import settings
from schedr.accounts import views as account_views

school = School.objects.get(name='uconn')
...
urlpatterns = patterns('schedr.school.views',
    ...
    (r'^(?P<user_code>[0-9a-f]{40})(?P<term_id>\d+)/$', 'public', {'school': school, 'Term': Term, 'UserData': UserData, 'Section': Section, 'template_name': 'school/uconn/public.html'}),
    ...
    (r'^(?P<term_name>.*?\d{4})/$', 'calendar', {'school': school, 'template_name': 'school/uconn/calendar.html', 'Term': Term, 'Major': Major, 'Section': Section, 'UserData': UserData, 'import_tab': settings.STUDENT_IMPORT, 'geneds': settings.GENEDS}),
    (r'^(?P<term_name>.*?\d{4})/save$', 'save', {'Section': Section, 'UserData': UserData, 'Term': Term}),
    (r'^(?P<term_name>.*?\d{4})/register$', 'finalize', {'template_name': 'school/uconn/register.html', 'Term': Term, 'school': school, 'UserData': UserData}),
